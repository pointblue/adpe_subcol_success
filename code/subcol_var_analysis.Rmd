...
list.of.packages <- c("tidyverse", "mgcv","Hmisc", "stringr","gridExtra","nlme", "raster","rgdal","sf","nngeo","bbmle","circular")
...
x <- pair_dat$`Aspect (°)`[pair_dat$col=="croz"]
y <- pair_dat$`Aspect ()`[pair_dat$col=="royds"]
watson.two.test(x,y)
...
r_ref = deviance(gam(prod_anom~1,data=r_mdat,method="ML"))
r_reft = deviance(rb)
r_denom = r_ref-r_reft

# run models with each covariate
r_area = gam(prod_anom~s(area,k=rk),sp=rb$sp[c(1,2)], method="REML", data = r_mdat)
r_pa <- gam(prod_anom~s(pa_ratio,k=rk),sp=rb$sp[c(3,4)], method="REML", data = r_mdat)
r_elev <- gam(prod_anom~s(adjust_mean_elev,k=rk),sp=rb$sp[c(5,6)], method="REML", data = r_mdat)
r_slope <- gam(prod_anom~s(mean_slope,k=rk),sp=rb$sp[c(7,8)], method="REML", data = r_mdat)
r_aspect <- gam(prod_anom~s(mean_aspect,k=rk),sp=rb$sp[9], method="REML", data = r_mdat)

r_area = gam(prod_anom~s(area,k=rk),sp=rb$sp[c(1,2)], method="ML", data = r_mdat)
r_pa <- gam(prod_anom~s(pa_ratio,k=rk),sp=rb$sp[c(3,4)], method="ML", data = r_mdat)
r_elev <- gam(prod_anom~s(adjust_mean_elev,k=rk),sp=rb$sp[c(5,6)], method="ML", data = r_mdat)
r_slope <- gam(prod_anom~s(mean_slope,k=rk),sp=rb$sp[c(7,8)], method="ML", data = r_mdat)
r_aspect <- gam(prod_anom~s(mean_slope,k=rk),sp=rb$sp[9], method="ML", data = r_mdat)


# calculate deviance of each covariate model
r_area_dev <- deviance(r_area)
r_pa_dev <- deviance(r_pa)
r_elev_dev <- deviance(r_elev)
r_slope_dev <- deviance(r_slope)
r_aspect_dev <- deviance(r_aspect)

# calculated r2 deviance
r_r2dev_area <- (r_ref-r_area_dev)/r_denom
r_r2dev_pa <- (r_ref-r_pa_dev)/r_denom
r_r2dev_elev <- (r_ref-r_elev_dev)/r_denom
r_r2dev_slope <- (r_ref-r_slope_dev)/r_denom
r_r2dev_aspect <- (r_ref-r_aspect_dev)/r_denom

r2dev_tab <-data.frame(term=c("area","par","elev","slope","aspect","wind","flood"),r_r2dev=c(r_r2dev_area,r_r2dev_pa,r_r2dev_elev,r_r2dev_slope,r_r2dev_aspect,NA,NA))
...
# redo statistical tests between colonies
pair_dat <-c_attr%>%
  full_join(r_attr)%>%
  # group_by(ID,col,attr)%>%
  # dplyr::slice(1)%>%
...
filter(pair_dat,col=="royds",samp==1)
...
watson.two.test(x,y) #test statistic 1.67, critical values 0.187, p<0.001
median(pair_dat$`Aspect (°)`[pair_dat$col=="croz"]) # 57.56574
median(pair_dat$`Aspect (°)`[pair_dat$col=="royds"]) # 193.6522 ()
#crozier mean aspect
mean.circular(x) #60.07
# royds mean aspect
mean.circular(y) #-52.05617
...
# 
# data:  c(croz_skua, royds_skua) out of c(n_croz, n_royds)
# X-squared = 33.56, df = 1, p-value = 6.91e-09
