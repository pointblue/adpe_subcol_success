...
subcol_attr%>%
  ...
  dplyr::slice(1)%>%
  filter(!attr%in%c("Skua 50m","Aspect (deg)"))%>%
...
?them# Note, added skua panel separately in illustrator because couldn't figure out how to do it easily in R
...
# Cv of each subcolony (did not use)
# r_cv_sub<-r_ct_all%>%
#   group_by(subcol)%>%
#   summarise(mean=mean(prod,na.rm=TRUE),sd=sd(prod,na.rm=TRUE))%>%
#   mutate(cv=sd/mean)
...
# r_ann_prod <- r_ct_all%>%
#   group_by(season)%>%
#   summarise(active_all=sum(active_ct),chx_all=sum(ch_ct),prod_all=chx_all/active_all)
# # CV
# sd(r_ann_prod$prod_all)/mean(r_ann_prod$prod_all)
...
r_seasonx<-  gam(prod_anom~s(area,by=season,k=rk) + s(pa_ratio, by=season,k=rk-1)+ s(adjust_mean_elev,by=season, k=rk)+s(mean_slope,by=season,k=rk)+s(mean_aspect,k=rk, bs="cc", by=season)+skua50*season, data=r_mdat, select=TRUE, method="ML")
gam.check(r_seasonx)
...
cb<-c_seasonx_anom # model for comparison
...
# area removed
cb0 <- update(cb,.~.-s(area, by=season, k=ck),sp=cb$sp[-c(1:8)])
#pa ratio removed
cb1 <-update(cb,.~.-s(pa_ratio, by=season, k=ck),sp=cb$sp[-c(3,4)])
summary(cb1)
plot(cb1,pages=1)
# slope removed
cb2 <- update(cb, .~.-s(mean_slope,k=ck),sp=cb$sp[-c(5,6)])
summary(cb2)
plot(cb2,pages=1)

# Aspect removed
cb3 <-update(cb,.~.-s(mean_aspect,k=ck, bs="cc"),sp=cb$sp[-7])
summary(cb3)
plot(cb3, pages=1)

# elevation removed
cb4 <-update(cb,.~.-s(adjust_mean_elev, k=ck),sp=cb$sp[-c(8,9)])
summary(cb4)
plot(cb2, pages=1)

# flow accumulation removed
cb5 <- update(cb,.~.-s(flow_acc_log1p, k=ck),sp=cb$sp[-c(10,11)])
summary(cb5)
plot(cb5,pages=1)

# windshelter already selected out
cb6 <- update(cb,.~.-s(mean_windshelt,k=ck),sp=cb$sp[-c(12,13)])
#Skua removed
cb7 <- update(cb,.~.-skua50,sp=cb$sp)

# Percent decrease in R2 when each variable removed
# c_area_r2 <-(summary(cb)$r.sq-summary(cb7)$r.sq)*100/summary(cb)$r.sq
c_pa_r2 <- (summary(cb)$r.sq-summary(cb1)$r.sq)*100/summary(cb)$r.sq
c_slope_r2 <- (summary(cb)$r.sq-summary(cb2)$r.sq)*100/summary(cb)$r.sq
c_aspect_r2 <- (summary(cb)$r.sq-summary(cb3)$r.sq)*100/summary(cb)$r.sq
c_elev_r2 <- (summary(cb)$r.sq-summary(cb4)$r.sq)*100/summary(cb)$r.sq
c_flood_r2<- (summary(cb)$r.sq-summary(cb5)$r.sq)*100/summary(cb)$r.sq
c_wind_r2 <- (summary(cb)$r.sq-summary(cb6)$r.sq)*100/summary(cb)$r.sq
c_skua_r2 <- (summary(cb)$r.sq-summary(cb7)$r.sq)*100/summary(cb)$r.sq

c_pa_dr2 <- (summary(cb)$r.sq-summary(cb1)$r.sq)
c_slope_dr2 <- (summary(cb)$r.sq-summary(cb2)$r.sq)
c_aspect_dr2 <- (summary(cb)$r.sq-summary(cb3)$r.sq)
c_elev_dr2 <- (summary(cb)$r.sq-summary(cb4)$r.sq)
c_flood_dr2<- (summary(cb)$r.sq-summary(cb5)$r.sq)
c_wind_dr2 <- (summary(cb)$r.sq-summary(cb6)$r.sq)
c_skua_dr2 <- (summary(cb)$r.sq-summary(cb7)$r.sq)
...
#pa ratio removed
rb1 <-update(rb,.~.-s(pa_ratio, k=rk),sp=rb$sp[-c(3,4)])
summary(rb1)
plot(rb1,pages=1)
# elevation removed
rb2 <-update(rb,.~.-s(adjust_mean_elev, k=rk),sp=rb$sp[-c(5,6)])
summary(rb2)
plot(rb2, pages=1)
# slope removed
rb3 <- update(rb, .~.-s(mean_slope,k=rk),sp=rb$sp[-c(7,8)])
summary(rb3)
plot(rb3,pages=1)
# Aspect already dropped but can confirm
rb4 <- update(rb, .~.-s(mean_aspect,k=rk, bs="cc"),sp=rb$sp[-c(9)])
summary(rb4)
plot(rb4,pages=1)
# flow accumulation removed
...
# plot(rb4,pages=1)
#Skua removed
rb5 <- update(rb,.~.-skua50,sp=rb$sp)
# area removed
rb6 <- update(rb,.~.-s(area,k=rk),sp=rb$sp[-c(1,2)])
summary(rb6)


# percent decrease in base R2 when each variable removed
r_area_r2 <-(summary(rb)$r.sq-summary(rb6)$r.sq)*100/summary(rb)$r.sq
...
r_aspect_r2 <- (summary(rb)$r.sq-summary(rb4)$r.sq)*100/summary(rb)$r.sq
# r_flood_r2<- (summary(rb)$r.sq-summary(rb4)$r.sq)*100/summary(rb)$r.sq
# calculate skua diff between smoother only model and all smoothers + skua (top_mod)
r_skua_r2 <- (summary(rb)$r.sq-summary(rb5)$r.sq)*100/summary(rb)$r.sq


r_area_dr2 <-(summary(rb)$r.sq-summary(rb6)$r.sq)
r_pa_dr2 <- (summary(rb)$r.sq-summary(rb1)$r.sq)
r_elev_dr2 <- (summary(rb)$r.sq-summary(rb2)$r.sq)
r_slope_dr2 <- (summary(rb)$r.sq-summary(rb3)$r.sq)
r_aspect_dr2 <- (summary(rb)$r.sq-summary(rb4)$r.sq)
# r_flood_r2<- (summary(rb)$r.sq-summary(rb4)$r.sq)*100/summary(rb)$r.sq
# calculate skua diff between smoother only model and all smoothers + skua (top_mod)
r_skua_dr2 <- (summary(rb)$r.sq-summary(rb5)$r.sq)
# 

# Make table of percent change in R2
rb_term_r2 <- round(c(r_area_r2,r_pa_r2,r_aspect_r2,r_slope_r2,NA,NA,r_elev_r2,r_skua_r2),1)
cb_term_r2 <- round(c(0,c_pa_r2,c_aspect_r2,c_slope_r2,c_flood_r2,c_wind_r2,c_elev_r2,c_skua_r2),1)

rb_term_dr2 <- round(c(r_area_dr2,r_pa_dr2,r_aspect_dr2,r_slope_dr2,NA,NA,r_elev_dr2,r_skua_dr2),3)
cb_term_dr2 <- round(c(0,c_pa_dr2,c_aspect_dr2,c_slope_dr2,c_flood_dr2,c_wind_dr2,c_elev_dr2,c_skua_dr2),)


term_r2_tab <-data.frame(term=c("Area","Perim-area_ratio","Aspect","Slope","flow_acc","Wind_shelter","Elevation","skua"), croz_delta_r2=cb_term_dr2,royds_delta_r2=rb_term_dr2,croz_perc_r2=cb_term_r2,royds_perc_r2 = rb_term_r2) 

term_r2_tab

write.csv(term_r2_tab, "data/delta_r2_tab.csv",row.names = FALSE)
...
```{r Predict productivity to at croz}
...
c_base_pred <- gam(prod_anom~s(area, k=k) + s(pa_ratio, k=k)+s(adjust_mean_elev, k=k)+ s(mean_slope, k=k) +s(mean_aspect,k=k, bs="cc") +s(mean_windshelt,k=k)+s(flow_acc_log1p, k=k)+skua50, select=TRUE, data=c_pred_dat, method="ML")
...
r_pred_dat <- filter(pred_dat,col=="royds")%>%
  mutate(skua50=as.numeric(skua50))
r_base_pred <- gam(prod_anom~s(area, k=k) + s(pa_ratio, k=k)+s(adjust_mean_elev, k=k)+ s(mean_slope, k=k)+skua50, select=TRUE, data=r_pred_dat, method="ML")
plot(r_base_pred,pages=1)
...
writeRaster(r_pred_anom, "Z:/Informatics/S031/analyses/aschmidt/subcol_var/GIS/royds/rev2/royds_predict_subcol.tif", format="GTiff", overwrite=TRUE)
...
c_subcol_t <- crop(projectRaster(c_subcol_qual, crs=proj),ext)
...
jpeg(file=paste("croz_covariates", "jpeg", sep = "."),units="in",width=11.5,height=14, res=300)
grid.arrange(p.c_aspect,p.c_slope,p.c_elev,p.c_flow,p.c_wind,p.c_subcol,
             ncol = 3, nrow = 4, 
             layout_matrix = rbind(c(1,2,3), c(4,6,6),c(5,6,6)))
dev.off()
...
jpeg(file=paste("royds_covariates", "jpeg", sep = "."),units="in",width=12,height=14, res=300)
grid.arrange(p.r_aspect,p.r_slope,p.r_elev,p.r_flow,p.r_wind,p.r_subcol,
             ncol = 3, nrow = 4, 
             layout_matrix = rbind(c(1,2,3), c(4,6,6),c(5,6,6)))
dev.off()
