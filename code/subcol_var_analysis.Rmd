...
c_base_anom <-gam(prod_anom~s(area, k=ck)+s(pa_ratio, k=ck)+s(mean_aspect,k=ck, bs="cc") +s(mean_windshelt,k=ck) +s(flow_acc_log1p,k=ck)+s(adjust_mean_elev,k=ck)+ s(mean_slope, k=ck)+skua50, data=filter(ann_prod_anom,col=="croz"), select=TRUE, method="REML")
...
# table with model structure
models<- data.frame(colony=c("Cape Crozier", "Cape Crozier","Cape Royds","Cape Royds","Cape Royds","Cape Royds"),model_name=c("m1","m2","m1","m2","m3","m4"), covariates=c("area + perimeter/area ratio + slope + aspect + elevation + flow accumulation + wind shelter index + skua within 50m","area + perimeter/area ratio + slope + aspect + elevation + flow accumulation + wind shelter index + skua within 50m + year interaction","area + perimeter/area ratio + slope + elevation + flow accumulation + wind shelter index + skua within 50m","area + perimeter/area ratio + slope + aspect + elevation + flow accumulation + skua within 50m","area + perimeter/area ratio + slope + elevation + flow accumulation + wind shelter index + skua within 50m + year interaction","area + perimeter/area ratio + slope + aspect + elevation + flow accumulation + skua within 50m + year interaction"))
 
models%>%
  gt(rowname_col="model_name",groupname_col = "colony")%>%
  cols_align("left")%>%
  tab_header(title="",subtitle = "Covariates: area + perimeter/area ratio + slope + aspect + elevation + flow accumulation + wind shelter index + skua within 50m")
...
cb0 <- gam(prod_anom~1, data=filter(ann_prod_anom,col=="croz"), method="ML")
summary(cb0)
...
# aspect removed
cb7<- gam(prod_anom~s(area, k=ck) + s(pa_ratio, k=ck)+s(adjust_mean_elev, k=ck)+ s(mean_slope, k=ck) +s(mean_windshelt,k=ck)+s(flow_acc_log1p, k=ck), data=filter(ann_prod_anom,col=="croz"),select=TRUE, method="ML",sp=c_base_anom$sp[-5])
...
c_aspect_ndevex <- ((deviance(cb7)-deviance(cb))/deviance(cb0))
