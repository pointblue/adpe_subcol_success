...
gam_wind_solar_aspect <- gam(prod~s(area, k=k, bs="cs")+s(pa_ratio, k=k, bs="cs")+s(mean_slope,k=k, bs="cs")+s(adjust_mean_elev,k=k, bs="cs")+s(mean_aspect,k=k, bs="cs")+s(mean_aspect,k=k, bs="cs")+s(mean_wind,k=k, bs="cs")+s(flood_risk, k=k, bs="cs")+skua50+season+s(subcol, bs="re"),data=dat, method = "REML")
summary(gam_wind_solar_aspect)


# Model selection on these
AICctab(gam_wind_solar_aspect,gam_aspect,gam_wind,gam_solar, gam_wind_solar,nobs=276, base=TRUE, weights=TRUE, logLik=TRUE)

# model with just wind is best model (solar gets dropped)

# run model with only variables with edf >0 from previous
...
# Got different results usingthis approach (grrr)
# Maybe because can't include random effect of subcol??
# Area retained as important in this model
