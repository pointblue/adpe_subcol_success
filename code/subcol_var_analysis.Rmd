...
# Need to check if have aspect issue at Croz####
# Yes! A few subcol span North
...
# because several subolonies face near north, aspects wrap around from 300 to 0 so average is not accurate
...
# Fit model with no interactions
r_base_anom <- gam(prod_anom~s(area, k=rk) + s(pa_ratio, k=rk)+s(adjust_mean_elev, k=rk)+s(mean_slope,k=rk) +s(mean_windshelt,k=rk)+s(mean_aspect,k=rk, bs="cc")+skua50, data=r_mdat, method="ML", select="TRUE",na.action="na.pass")
summary(r_base_anom)
...
r_base_wind <- gam(prod_anom~s(area, k=rk) + s(pa_ratio, k=rk)+s(adjust_mean_elev, k=rk)+s(mean_slope,k=rk) +s(mean_windshelt,k=rk)+skua50, data=filter(ann_prod_anom,col=="royds"), select=TRUE, method="ML")
gam.check(r_base_wind)
summary(r_base_wind)
plot.gam(r_base_wind, pages=1, shade=T, residuals=T)

r_base_pa <-update(r_base_anom,.~.-s(pa_ratio,k=rk))
summary(r_base_pa)

r_base_area <-update(r_base_anom,.~.-s(area,k=rk))
summary(r_base_area)
plot(r_base_area,pages=1)

t <- MuMIn::dredge(r_base_anom)
t2 <- MuMIn::dredge(r_base_anom)

r_base_aspect <- gam(prod_anom~s(area, k=rk) + s(pa_ratio, k=rk)+s(adjust_mean_elev, k=rk)+s(mean_slope,k=rk) +s(mean_aspect,k=rk, bs="cc")+s(flow_acc_log1p,k=rk)+skua50, data=filter(ann_prod_anom,col=="royds"), select=TRUE, method="ML")
gam.check(r_base_aspect)
concurvity(r_base_aspect)
summary(r_base_aspect)
plot.gam(r_base_aspect, pages=1, shade=T, residuals=T)

t3 <- gam(prod_anom~s(area, k=rk) + s(pa_ratio, k=rk)+s(mean_aspect,k=rk, bs="cc")+s(adjust_mean_elev, k=rk), data=filter(ann_prod_anom,col=="royds"), select=TRUE, method="REML")
summary(t3)
t4 <- gam(prod_anom~s(area, k=rk) + s(pa_ratio, k=rk)+s(mean_slope,k=rk) +s(mean_windshelt,k=rk)+s(mean_aspect,k=rk, bs="cc"), data=r_mdat, method="REML", select="TRUE",na.action="na.pass")
...
#removed flow
r_base_t1 <- gam(prod_anom~s(area, k=rk) + s(pa_ratio, k=rk)+s(adjust_mean_elev, k=rk)+s(mean_slope,k=rk) +s(mean_windshelt,k=rk)+s(mean_aspect,k=rk, bs="cc")+skua50, data=filter(ann_prod_anom,col=="royds"), method="REML", select="TRUE")
round(concurvity(r_base_t1, full=FALSE)$estimate,2)

# removed area
r_base_t2 <- gam(prod_anom~s(pa_ratio, k=rk)+s(adjust_mean_elev, k=rk)+s(mean_slope,k=rk) +s(mean_windshelt,k=rk)+s(mean_aspect,k=rk, bs="cc") +s(flow_acc_log1p,k=rk)+skua50, data=filter(ann_prod_anom,col=="royds"), method="REML", select="TRUE")

round(concurvity(r_base_t2, full=FALSE)$estimate,2)

# remove pa-ratio
r_base_t3 <- gam(prod_anom~s(area, k=rk)+s(adjust_mean_elev, k=rk)+s(mean_slope,k=rk) +s(mean_windshelt,k=rk)+s(mean_aspect,k=rk, bs="cc") +s(flow_acc_log1p,k=rk)+skua50, data=filter(ann_prod_anom,col=="royds"), method="REML", select="TRUE")
summary(r_base_t3)


# model with variables that <0.5 pairwise concurvity with pa-ratio
r_base_t4 <- gam(prod_anom~s(pa_ratio, k=rk) +s(mean_windshelt,k=rk)+s(mean_aspect,k=rk, bs="cc") +s(flow_acc_log1p,k=rk)+skua50, data=filter(ann_prod_anom,col=="royds"), method="REML", select="TRUE")
summary(r_base_t4)
plot(r_base_t4, pages=1)
...
r_seasonx_aspect <-  gam(prod_anom~s(area,by=season,k=rk) + s(pa_ratio,by=season,k=rk)+ s(adjust_mean_elev,by=season, k=rk)+s(mean_slope,by=season,k=rk) +s(mean_aspect, by=season,bs="cc", k=rk) + s(flow_acc_log1p,by=season,k=rk)+ skua50*season, data=filter(ann_prod_anom,col=="royds"), select=TRUE, method="ML")
...
library(bbmle)
AICctab(r_base_anom,r_base_wind,r_base_aspect,r_seasonx_aspect,nobs=86, base=TRUE, weights=TRUE, logLik=TRUE)
...
#Skua removed
cb6 <- update(c_base_anom,.~.-skua50,sp=c_base_anom$sp)
...
rb <- update(r_base_aspect,.~.-skua50,sp=r_base_aspect$sp)
plot(rb,pages=1)
...
# plot(rb4, pages=1)
# flow accumulation removed
rb5 <- update(rb,.~.-s(flow_acc_log1p, k=rk),sp=top_mod$sp[-c(10,11)])
summary(rb5)
plot(rb5,pages=1)
#Skua removed
...
rb_term_ndevex <- round(c(0,r_pa_ndevex,r_elev_ndevex,r_slope_ndevex,0,NA,r_flood_ndevex,r_skua_ndevex),3)*100
sum(rb_term_ndevex, na.rm=TRUE)

cb_term_ndevex<- round(c(0,c_pa_ndevex,c_elev_ndevex, c_slope_ndevex,c_aspect_ndevex, 0,c_flood_ndevex, c_skua_ndevex),3)*100
sum(cb_term_ndevex)
