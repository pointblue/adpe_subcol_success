...
subcol_attr <- all_dat%>%
  ...
  mutate(measurement=as.numeric(measurement),attr=factor(attr, labels=c("Elevation (m)","Area (m2)","Flow accumulation (ln)","Aspect (deg)","Slope (deg)","Wind shelter index","Perim/Area ratio", "Skua 50m")),
           attr=ordered(attr,levels=c("Area (m2)","Perim/Area ratio","Aspect (deg)","Slope (deg)","Flow accumulation (ln)","Wind shelter index","Elevation (m)", "Skua 50m")))%>%
  filter(!is.na(measurement))

# label panels with significant difference
attr_sig <- data.frame(attr=ordered(c("Area (m2)","Perim/Area ratio","Aspect (deg)","Slope (deg)","Flow accumulation (ln)","Wind shelter index","Elevation (m)", "Skua 50m")), sig=as.character(c("*","*","*","","*","*","*","*")))
...
cb<-c_base_anom # model for comparison with skua removed
...
#pa ratio removed
cb1 <-update(cb,.~.-s(pa_ratio, k=ck),sp=cb$sp[-c(3,4)])
summary(cb1)
plot(cb1,pages=1)
# slope removed
...
rb_term_r2 <- round(c(r_area_r2,r_pa_r2,r_elev_r2,r_slope_r2,r_aspect_r2,NA,NA,r_skua_r2),1)
cb_term_r2 <- round(c(0,c_pa_r2,c_elev_r2,c_slope_r2,c_aspect_r2,c_wind_r2,c_flood_r2,c_skua_r2),1)

term_r2_tab <-data.frame(term=c("Area","Perim/area_ratio","Elevation","Slope","Aspect","Wind_shelter", "flow_acc","skua"), croz_delta_r2=cb_term_r2,royds_delta_r2 = rb_term_r2) 
term_r2_tab
...
if(add==FALSE){
plot(plot_dat[,x],plot_dat[,"fit_var"],ylim=ylim,xlim=xlim,add=add, xlab=xlab,ylab=ylab, type="n",cex.lab=1.4)
...
# dev.new()
# cairo_ps()
par(mfrow=c(4,2),mar=c(4,6,3,1),mgp=c(2.1,0.4,0), las=1, tck=-0.02)
ylim=c(-.5,0.45)
...
plot.smooths(x="area",data=pred_dat[pred_dat$col=="royds",],model=r_base_pred,add=FALSE,ylim=ylim,xlab=expression(Area~(m^2)),col_x=col2)
...
var="mean_slope"
xlim <- c(min(pred_dat[pred_dat$col=="croz",var],pred_dat[pred_dat$col=="royds",var]),max(pred_dat[pred_dat$col=="croz",var],pred_dat[pred_dat$col=="royds",var]))
plot.smooths(x="mean_slope",data=pred_dat[pred_dat$col=="croz",],model=c_base_pred,add=FALSE,ylim=ylim,xlim=xlim,xlab="Slope (degrees)",col_x=col1)
...
# xlim <- c(min(pred_dat[pred_dat$col=="croz",var],pred_dat[pred_dat$col=="royds",var]),max(pred_dat[pred_dat$col=="croz",var],pred_dat[pred_dat$col=="royds",var]))
xlim=c(-150,150)
plot.smooths(x="mean_aspect",data=pred_dat[pred_dat$col=="croz",],model=c_base_pred,add=FALSE,ylim=ylim,xlim=xlim,xlab="Aspect (degrees)",col_x=col1)
...
skua_mod<-ggplot(cr_skua,aes(skua50,coef,col=col))+
  ...
  geom_errorbar(aes(ymin=coef-ci,ymax=coef+ci),width=0.05, size=0.5,position=position_dodge(width=0.15))+
  theme_bw()+
...
```{r Predict productivity to space}
...
k=4
r_base_pred <- gam(prod_anom~s(area, k=k) + s(pa_ratio, k=k)+s(adjust_mean_elev, k=k)+ s(mean_slope, k=k)+skua50, select=TRUE, data=filter(pred_dat,col=="royds"), method="ML")
plot(r_base_pred,pages=1)
