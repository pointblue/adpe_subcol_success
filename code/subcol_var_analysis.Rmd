...
# c_ann_prod<-c_ct_meas%>%
#   group_by(season)%>%
#   summarise(active_all=sum(active_ct,na.rm=TRUE),chx_all=sum(ch_ct,na.rm=TRUE),prod_all=chx_all/active_all)
# # CV
# sd(c_ann_prod$prod_all)/mean(c_ann_prod$prod_all)


# Mean and SE of chicks per pair for whole time series (not used in ms)
# sd(c_ct_meas$prod,na.rm=TRUE)/sqrt(length(c_ct_meas$prod)-5) # 5 NA's
# # mean(c_dat$prod, na.rm=TRUE)
# mean(c_ann_prod$prod_all)
# # Croz SE
# sd(c_ann_prod$prod_all)/sqrt(length(c_ann_prod$prod_all))
# # Royds SE
# sd(r_ann_prod$prod_all)/sqrt(length(r_ann_prod$prod_all))
# 
...
# Coefficient of variation for four years (did not use)
# Royds
...
# flow at Royds has non-zero estimates at only 3 subcolonies, doesn't seem like a good idea to include
# Fit model with no interactions
...
r_seasonx<-  gam(prod_anom~s(area,by=season,k=rk) + s(pa_ratio, by=season,k=rk)+ s(adjust_mean_elev,by=season, k=rk)+s(mean_slope,by=season,k=rk)+skua50*season, data=r_mdat, select=TRUE, method="ML")
gam.check(r_seasonx)
...
rb_term_r2 <- round(c(r_area_r2,r_pa_r2,r_elev_r2,r_slope_r2,NA,NA,r_flood_ndevex,r_skua_ndevex),3)
# rb_term_r2 <- round(c(0,r_pa_r2,r_elev_r2,r_slope_r2,0,NA,r_flood_r2,r_skua_r2),1)
# 
...
# calculate percent increase in correlation between observed versus predicted when variable in question is added
rcorr(croz_ann_prod$prod_anom,croz_ann_prod$predict_base)
rcor_base <- rcorr(royds_ann_prod$prod_anom,royds_ann_prod$predict_base)$r[1,2]
r_pa_dif <- (rcor_base-rcorr(royds_ann_prod$prod_anom,royds_ann_prod$p_rb1)$r[1,2])/rcorr(royds_ann_prod$prod_anom,royds_ann_prod$p_rb1)$r[1,2] # pa_ratio
r_elev_dif<- (rcor_base-rcorr(royds_ann_prod$prod_anom,royds_ann_prod$p_rb2)$r[1,2])/rcorr(royds_ann_prod$prod_anom,royds_ann_prod$p_rb2)$r[1,2] #elevation
r_slope_dif <- (rcor_base-rcorr(royds_ann_prod$prod_anom,royds_ann_prod$p_rb3)$r[1,2])/ rcorr(royds_ann_prod$prod_anom,royds_ann_prod$p_rb3)$r[1,2] # slope
# r_flow_dif <- (rcor_base-rcorr(royds_ann_prod$prod_anom,royds_ann_prod$p_rb4)$r[1,2])/ rcorr(royds_ann_prod$prod_anom,royds_ann_prod$p_rb4)$r[1,2] # flow
r_skua_dif <- (rcor_base-rcorr(royds_ann_prod$prod_anom,royds_ann_prod$p_rb5)$r[1,2])/ rcorr(royds_ann_prod$prod_anom,royds_ann_prod$p_rb5)$r[1,2] #skua 
r_area_dif <- (rcor_base-rcorr(royds_ann_prod$prod_anom,royds_ann_prod$p_rb6)$r[1,2])/ rcorr(royds_ann_prod$prod_anom,royds_ann_prod$p_rb6)$r[1,2] #area 




rcorr(royds_ann_prod$prod_anom,royds_ann_prod$p_rb7) # area
# 
# 
