...
subcol_attr <- all_dat%>%
  ...
  mutate(measurement=as.numeric(measurement),attr=factor(attr, labels=c("Elevation (m)","Area (m2)","Flow accumulation","AreaM","Aspect (deg)","Slope (deg)","Wind shelter index","Perim/Area ratio", "Skua 50m")))%>%
...
# Having trouble making the skua panel so doingn it manually here
...
ggplot(aes(col, proportion, fill=col))+ 
...
p1_cbqi <-ggplot(c_bqi_pred_prod,aes(pred_prod,BQI_A, color=bqi_cat))+
  ...
  # theme_bw()+ 
  guides(colour=guide_legend(title="Crozier\nBQI\ncategory"), fill=guide_legend(title="Crozier\nBQI\ncategory"))+
  theme_classic()+
  ggtitle(("A"))+
...
c_subcol <- read.csv("Z:/Informatics/S031/analyses/aschmidt/subcol_var/data/croz_all_subcol_2014.txt")%>%
  mutate(r=sqrt(area/pi),pa_circ=2/r,resid=pa_ratio-pa_circ)

ggplot(c_subcol,aes(area,resid))+
  geom_point()+
  geom_smooth(method="loess")

ggplot(c_subcol,aes(area, pa_ratio))+
  geom_point()+
  geom_line(aes(area,pa_circ), col="blue")+
  ylim(0,4)+
  xlim(0,5000)+
