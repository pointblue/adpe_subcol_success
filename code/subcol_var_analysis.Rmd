...
# Fit model with no interactions
 # +s(mean_aspect,k=ck, bs="cc") ,+s(mean_windshelt,k=ck),s(area, k=ck), +s(flow_acc_log1p, k=ck)
c_base_anom <-gam(prod_anom~s(pa_ratio, k=ck)+s(adjust_mean_elev, k=ck)+ s(mean_slope, k=ck)+skua50, data=filter(ann_prod_anom,col=="croz"), select=TRUE, method="ML")

# c_base_anom <-gam(prod_anom~s(area, k=ck) + s(pa_ratio, k=ck)+s(adjust_mean_elev, k=ck)+ s(mean_slope, k=ck) +s(mean_windshelt,k=ck)+s(flow_acc_log1p, k=ck)+skua50, data=filter(ann_prod_anom,col=="croz"), select=TRUE, method="ML")
# 
# c_base_anom <-gam(prod_anom~s(area, k=ck) + s(pa_ratio, k=ck)+s(adjust_mean_elev, k=ck)+ s(mean_slope, k=ck) +s(mean_aspect,k=ck, bs="cc")+s(flow_acc_log1p, k=ck)+skua50, data=filter(ann_prod_anom,col=="croz"), select=TRUE, method="ML")
...
c_base_comp <- gam(prod_anom~ s(pa_ratio, k=ck)+s(adjust_mean_elev, k=ck)+ s(mean_slope, k=ck) +s(mean_aspect,k=ck, bs="cc")+s(flow_acc_log1p, k=ck)+skua50, data=filter(ann_prod_anom,col=="croz"), select=TRUE, method="ML")
summary(c_base_comp)
plot.gam(c_base_comp,pages=1)
...
# 
# c_seasonx_anom <-gam(prod_anom~ s(pa_ratio, by=season, k=ck)+s(adjust_mean_elev,by=season, k=ck)+s(mean_slope, by=season, k=ck) +s(mean_aspect,by=season,k=ck, bs="cc") +s(mean_windshelt,by=season,k=ck)+s(flow_acc_log1p,by=season, k=ck)+skua50*season, data=filter(ann_prod_anom,col=="croz"), select=TRUE, method="ML")
# c_seasonx_anom <-gam(prod_anom~s(area, by=season, k=ck) + s(pa_ratio, by=season, k=ck)+s(adjust_mean_elev,by=season, k=ck)+s(mean_slope, by=season, k=ck) +s(mean_windshelt,by=season,k=ck)+s(flow_acc_log1p,by=season, k=ck)+skua50*season, data=filter(ann_prod_anom,col=="croz"), select=TRUE, method="ML")
# plot(c_seasonx_anom, pages=4)
...
Gams for Royds
```{r Royds Gams, include=FALSE}

# set k for royds
rk=4
# Fit model with no interactions
r_base_anom <- gam(prod_anom~s(area, k=rk) + s(pa_ratio, k=rk)+s(adjust_mean_elev, k=rk)+s(mean_slope,k=rk) +s(mean_windshelt,k=rk)+s(mean_aspect,k=rk, bs="cc") +s(flow_acc_log1p, k=rk)+skua50, data=filter(ann_prod_anom,col=="royds"), method="ML", select="TRUE")
gam.check(r_base_anom) # looks pretty good, some skew in the residuals
concurvity(r_base_anom, full=FALSE) # nothing above 0.6
...
summary(r_seasonx_anom_2)


library(bbmle)
AICctab(r_base_anom,r_base_anom_1,r_base_anom_2,r_seasonx_anom_1,r_seasonx_anom_2,nobs=86, base=TRUE, weights=TRUE, logLik=TRUE)
