...
r_base_wind <- gam(prod_anom~s(area, k=rk) + s(pa_ratio, k=rk)+s(adjust_mean_elev, k=rk)+s(mean_slope,k=rk) +s(mean_windshelt,k=rk)+skua50, data=r_mdat, select=TRUE, method="ML")
...
# #skua p >0.1 checking if affects model ranking if remove [no]
r_base_aspect_re <- gam(prod_anom~s(area, k=rk) + s(pa_ratio, k=rk)+s(adjust_mean_elev, k=rk)+s(mean_slope,k=rk) +s(mean_aspect,k=rk, bs="cc"), data=r_mdat, select=TRUE, method="ML")
summary(r_base_aspect_re)
...
#skua p >0.1 checking if affects model ranking if remove [no]
# r_seasonx_re<-  gam(prod_anom~s(area,by=season,k=rk) + s(pa_ratio, by=season,k=rk-1)+ s(adjust_mean_elev,by=season, k=rk)+s(mean_slope,by=season,k=rk)+s(mean_aspect,k=rk, bs="cc", by=season), data=r_mdat, select=TRUE, method="ML")
