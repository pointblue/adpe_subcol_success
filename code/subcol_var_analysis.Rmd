...
# Crozier
# v17 has windshelter calculated from windshelter_300m_pi_pi8_lcc169
c_ct_meas <- read_csv("data/croz_selected_meas_ct_all_v17.csv")
...
attr_sig <- data.frame(attr=factor(c("Elevation (m)","Area (m2)","Flow accumulation (log)","Aspect (deg)","Slope (deg)","Wind shelter index","Perim/Area ratio"), levels=c("Elevation (m)","Area (m2)","Flow accumulation","AreaM","Aspect (deg)","Slope (deg)","Wind shelter index","Perim/Area ratio")), sig=as.character(c("*","*","*","*","","*","*")))
...
c_base_anom <-gam(prod_anom~s(area, k=ck)+s(pa_ratio, k=ck)+s(mean_slope, k=ck)+s(mean_aspect,k=ck, bs="cc") +s(adjust_mean_elev,k=ck)+s(flow_acc_log1p,k=ck)+ s(mean_windshelt,k=ck)+skua50, data=filter(ann_prod_anom,col=="croz"), select=TRUE, method="REML")
# check if basis dimensions sifficient
par(mfrow=c(2,2))
gam.check(c_base_anom) # flow p-value is significatn, but edf not close to k so not adjusting
summary(c_base_anom)
plot(c_base_anom,pages=1, residuals=T)
concurvity(c_base_anom)
round(concurvity(c_base_anom, full=FALSE)$estimate,2) # nothing over 0.49
par(mfrow=c(2,2))
gam.check(c_base_anom)
round(concurvity(c_base_area, full=FALSE)$worst,2)

# Fit model with seasonal interaction
c_seasonx_anom <-gam(prod_anom~s(area, by=season, k=ck) + s(pa_ratio, by=season, k=ck)+s(adjust_mean_elev,by=season, k=ck)+s(mean_slope, by=season, k=ck) +s(mean_aspect,by=season,k=ck, bs="cc") +s(mean_windshelt,by=season,k=ck)+s(flow_acc_log1p,by=season)+skua50*season, data=filter(ann_prod_anom,col=="croz"), select=TRUE, method="REML")
...
rk=4
# Fit model with no interactions
r_base_anom <- gam(prod_anom~s(area, k=rk) + s(pa_ratio, k=rk)+s(adjust_mean_elev, k=rk)+s(mean_slope,k=5) +s(mean_windshelt,k=rk)+s(mean_aspect,k=rk, bs="cc") +s(flow_acc_log1p,k=rk)+skua50, data=filter(ann_prod_anom,col=="royds"), method="REML", select="TRUE")
summary(r_base_anom)

gam.check(r_base_anom) # looks pretty good, some skew in the residuals
round(concurvity(r_base_anom, full=FALSE)$estimate,2) # nothing above 0.58
plot.gam(r_base_anom, pages=1)
summary(r_base_anom)

# model above is behaving oddly with respect to windshelter and pa-ratio
# windshelter and aspect are pretty highly correlated (was -0.7 before, now -0.64)
# so can justify running models with each independently
r_base_wind <- gam(prod_anom~s(area, k=rk) + s(pa_ratio, k=rk)+s(adjust_mean_elev, k=rk)+s(mean_slope,k=rk) +s(mean_windshelt,k=rk) +s(flow_acc_log1p, k=rk)+skua50, data=filter(ann_prod_anom,col=="royds"), select=TRUE, method="REML")
gam.check(r_base_wind)
summary(r_base_wind)
plot.gam(r_base_wind, pages=1, shade=T, residuals=T)


r_base_aspect <- gam(prod_anom~s(area, k=rk) + s(pa_ratio, k=rk)+s(adjust_mean_elev, k=rk)+s(mean_slope,k=rk) +s(mean_aspect,k=rk, bs="cc") +s(flow_acc_log1p, k=rk)+skua50, data=filter(ann_prod_anom,col=="royds"), select=TRUE, method="REML")
gam.check(r_base_aspect)
summary(r_base_aspect)
plot.gam(r_base_aspect, pages=1, shade=T, residuals=T)

r_base_t1 <- gam(prod_anom~s(area, k=rk) +s(adjust_mean_elev, k=rk)+s(mean_slope,k=rk) +s(mean_aspect,k=rk, bs="cc") +s(flow_acc_log1p, k=rk)+skua50, data=filter(ann_prod_anom,col=="royds"), select=TRUE, method="REML")
concurvity(r_base_t1)
summary(r_base_t1)
plot(r_base_t1, pages=1)

r_base_t2 <- gam(prod_anom~s(area, k=rk) +s(adjust_mean_elev, k=rk)+s(pa_ratio,k=rk) +s(mean_aspect,k=rk, bs="cc") +s(flow_acc_log1p, k=rk)+skua50, data=filter(ann_prod_anom,col=="royds"), select=TRUE, method="REML")
concurvity(r_base_t2)
summary(r_base_t2)
plot(r_base_t2, pages=1)




# fit model with seasonal interactions
r_seasonx_wind <-  gam(prod_anom~s(area,by=season,k=rk) + s(pa_ratio, by=season,k=rk)+ s(adjust_mean_elev,by=season, k=rk)+s(mean_slope,by=season,k=rk) +s(mean_windshelt,by=season,k=rk)+ s(flow_acc_log1p,by=season,k=3)+ skua50*season, data=filter(ann_prod_anom,col=="royds"), select=TRUE, method="REML")

summary(r_seasonx_anom_wind)
plot(r_seasonx_anom, pages=2, shade=TRUE)

r_seasonx_aspect <-  gam(prod_anom~s(area,by=season,k=rk) + s(pa_ratio,by=season,k=rk)+ s(adjust_mean_elev,by=season, k=rk)+s(mean_slope,by=season,k=rk) +s(mean_aspect, by=season,bs="cc", k=rk) + s(flow_acc_log1p,by=season,k=3)+ skua50*season, data=filter(ann_prod_anom,col=="royds"), select=TRUE, method="REML")
summary(r_seasonx_anom_aspect)

r_seasonx_anom<-  gam(prod_anom~s(area,by=season,k=rk) + s(pa_ratio, by=season,k=rk)+ s(adjust_mean_elev,by=season, k=rk)+s(mean_slope,by=season,k=rk) +s(mean_aspect,k=rk, bs="cc")+s(mean_windshelt,by=season,k=rk)+ s(flow_acc_log1p,by=season,k=rk)+ skua50*season, data=filter(ann_prod_anom,col=="royds"), select=TRUE, method="REML")
gam.check(r_seasonx_anom)

library(bbmle)
AICctab(r_base_t1,r_base_t2, r_base_wind,r_base_aspect,r_seasonx_wind,r_seasonx_aspect,nobs=86, base=TRUE, weights=TRUE, logLik=TRUE)


# make model output table with gt package ####
library(gt)
r_mod_list <- list(r_base_wind,r_base_aspect,r_seasonx_wind,r_seasonx_aspect)
r_mod_names <-c("r_base_wind","r_base_aspect","r_seasonx_wind","r_seasonx_aspect")

# make table with % dev explained to join to model table
r_devexpl <- data.frame(Model=c("m1", "m2","m3","m4"),dev.expl=round(c(summary(r_mod_list[[1]])$dev.expl, summary(r_mod_list[[2]])$dev.expl,summary(r_mod_list[[3]])$dev.expl,summary(r_mod_list[[4]])$dev.expl),2),
                        description=c("base (-aspect)", "base (-wind shelter)", " m1 x year", "m2 x year"))

rmod_tab <- as.data.frame(AICctab(r_mod_list,nobs=86, weights=TRUE, base=TRUE,logLik=TRUE, mnames=c("m1", "m2","m3","m4")))%>%
...
# order of variables must match base model

# Drop pa ratio
cb1 <-gam(prod_anom~s(area, k=ck)+s(mean_slope, k=ck)+s(mean_aspect,k=ck, bs="cc") +s(adjust_mean_elev,k=ck)+s(flow_acc_log1p,k=ck)+ s(mean_windshelt,k=ck)+skua50, data=filter(ann_prod_anom,col=="croz"),select=TRUE, method="REML",sp=cb$sp[-c(3,4)]) 

plot.gam(cb1,pages=1)

# slope removed
cb2<- gam(prod_anom~s(area, k=ck)+s(pa_ratio, k=ck)+s(mean_aspect,k=ck, bs="cc") +s(adjust_mean_elev,k=ck)+s(flow_acc_log1p,k=ck)+ s(mean_windshelt,k=ck)+skua50, data=filter(ann_prod_anom,col=="croz"), select=TRUE, method="REML",sp=cb$sp[-c(5,6)])

# aspect removed
cb3<- gam(prod_anom~s(area, k=ck)+s(pa_ratio, k=ck)+s(mean_slope, k=ck) +s(adjust_mean_elev,k=ck)+s(flow_acc_log1p,k=ck)+ s(mean_windshelt,k=ck)+skua50, data=filter(ann_prod_anom,col=="croz"), select=TRUE, method="REML",sp=cb$sp[-7])

# Fit model with elevation removed
cb4 <-gam(prod_anom~s(area, k=ck)+s(pa_ratio, k=ck)+s(mean_slope, k=ck)+s(mean_aspect,k=ck, bs="cc")+s(flow_acc_log1p,k=ck)+ s(mean_windshelt,k=ck)+skua50, data=filter(ann_prod_anom,col=="croz"), select=TRUE, method="REML",sp=cb$sp[-c(8,9)])

# Windshelter removed
# cb4 <- gam(prod_anom~s(area, k=ck) + s(pa_ratio, k=ck)+s(adjust_mean_elev, k=ck)+ s(mean_slope, k=ck) +s(mean_aspect,k=ck, bs="cc")+s(flow_acc_log1p, k=ck)+skua50, data=filter(ann_prod_anom,col=="croz"), select=TRUE,method="REML", sp=c_base_anom$sp[-c(6,7)])
# flow accumulation removed
cb5 <-gam(prod_anom~s(area, k=ck)+s(pa_ratio, k=ck)+s(mean_slope, k=ck)+s(mean_aspect,k=ck, bs="cc") +s(adjust_mean_elev,k=ck)+ s(mean_windshelt,k=ck)+skua50, data=filter(ann_prod_anom,col=="croz"), select=TRUE, method="REML", sp=cb$sp[-c(10,11)])
#Skua removed
cb6<-gam(prod_anom~s(area, k=ck)+s(pa_ratio, k=ck)+s(mean_slope, k=ck)+s(mean_aspect,k=ck, bs="cc") +s(adjust_mean_elev,k=ck)+s(flow_acc_log1p,k=ck)+ s(mean_windshelt,k=ck), data=filter(ann_prod_anom,col=="croz"), select=TRUE, method="REML", sp=cb$sp)
# # subcol ID removed
...
summary(cb)$dev.expl-summary(cb6)$dev.expl
...
# c_aspect_devex <- 1-((deviance(cb7)-deviance(cb))/deviance(cb0))

# Croz Proportion (deviance(rb1)-deviance(rb))/deviance(rb0)

c_pa_ndevex <- ((deviance(cb1)-deviance(cb))/deviance(cb0)) # same as summary(cb)$dev.expl-summary(cb1)$dev.expl
c_slope_ndevex <- ((deviance(cb2)-deviance(cb))/deviance(cb0))
c_aspect_ndevex <- ((deviance(cb3)-deviance(cb))/deviance(cb0))
c_elev_ndevex <- ((deviance(cb4)-deviance(cb))/deviance(cb0))
...
#pa ratio removed
rb1 <-gam(prod_anom~ s(area, k=rk)+s(adjust_mean_elev, k=rk)+ s(mean_slope, k=rk) +s(mean_aspect,k=rk, bs="cc")+s(flow_acc_log1p, k=rk)+skua50, data=filter(ann_prod_anom,col=="royds"),select=TRUE,method="ML", sp=rb$sp[-c(3,4)])
summary(rb1)

# Fit model with slope removed
rb2 <-gam(prod_anom~s(area, k=rk) + s(pa_ratio, k=rk)+ s(adjust_mean_elev, k=rk) +s(mean_aspect,k=rk, bs="cc")+s(flow_acc_log1p, k=rk)+skua50, data=filter(ann_prod_anom,col=="royds"),select=TRUE, method="ML", sp=rb$sp[-c(7,8)])
# Aspect removed
rb3 <-gam(prod_anom~s(area, k=rk) + s(pa_ratio, k=rk)+ s(adjust_mean_elev, k=rk) +s(mean_slope, k=rk)+s(flow_acc_log1p, k=rk)+skua50, data=filter(ann_prod_anom,col=="royds"),select=TRUE, method="ML", sp=rb$sp[-9])
# Windshelter removed
...
# flow accumulation removed
rb5 <- gam(prod_anom~s(area, k=rk) + s(pa_ratio, k=rk)+s(adjust_mean_elev, k=rk)+ s(mean_slope, k=rk) +s(mean_aspect,k=rk, bs="cc")+skua50, data=filter(ann_prod_anom,col=="royds"),select=TRUE, method="ML", sp=rb$sp[-c(12,13)] )
#Skua removed
rb6 <- gam(prod_anom~s(area, k=rk) + s(pa_ratio, k=rk)+s(adjust_mean_elev, k=rk)+ s(mean_slope, k=rk) +s(mean_aspect,k=rk, bs="cc")+s(flow_acc_log1p, k=rk), data=filter(ann_prod_anom,col=="royds"),select=TRUE, method="ML",sp=rb$sp[-14])
# area removed
rb7 <- gam(prod_anom~s(pa_ratio, k=rk)+s(adjust_mean_elev, k=rk)+ s(mean_slope, k=rk) +s(mean_aspect,k=rk, bs="cc")+s(flow_acc_log1p, k=rk), data=filter(ann_prod_anom,col=="royds"),select=TRUE, method="ML",sp=rb$sp[-c(1,2)])
...
rb_term_devex<-c(r_area_devex,r_pa_devex,0,r_slope_devex,r_aspect_devex,0,r_flood_devex,r_skua_devex)
sum(rb_term_devex)
rb_term_ndevex <- c(r_area_ndevex,r_pa_ndevex,0,r_slope_ndevex,r_aspect_ndevex,NA,r_flood_ndevex,r_skua_ndevex)
sum(rb_term_ndevex)

cb_term_devex<- c(0,c_pa_devex,c_elev_devex, c_slope_devex,c_aspect_devex,0, c_flood_devex, c_skua_devex)
sum(cb_term_devex)

cb_term_ndevex<- c(0,c_pa_ndevex,c_elev_ndevex, c_slope_ndevex,c_aspect_ndevex, 0,c_flood_ndevex, c_skua_ndevex)
sum(cb_term_ndevex)
