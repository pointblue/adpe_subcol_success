...
# Crozier
c_ct_meas <- read.csv("data/croz_selected_meas_ct_all_v15.csv")

# Royds
r_ct_meas <- read_csv("data/royds_selected_meas_ct_all_v8.csv")
...
# select only years 1415-1718  # remove s3 because it's an outlier
  filter(season%in%c("1415","1516","1617","1718"),!subcol=="s3",!prod=="NA"&!is.na(pa_ratio),!active_ct<5)%>%
  rename(mean_windshelt=mean_windshelt300m)

# Format Royds data
r_dat <- r_ct_meas%>%
  dplyr::select(-FID,-perimeter,-mean_elev)%>%
  mutate(season=factor(season),subcol=factor(subcol),col=factor(col),skua50_yn=factor(ifelse(skua50==1,"y","n")),skua50=factor(skua50))%>%
...
# # wind shelt to 0.1097
# 0.1538
r_dat[r_dat$subcol=="1b","mean_windshelt"]=0.1538
# aspect to 253.100
...
# missing a chunk of code defining cr_skua
p_skua <- ggplot(cr_skua, aes(col,prop_skua, fill=col))+
  geom_bar(stat="identity")+
   xlab("Skua within 50m")+
  ylab("Proportion of subcolonies")+
  # scale_fill_viridis(discrete=TRUE)+
  scale_color_manual(values=c(col1,col2),labels = c("Crozier","Royds"))+
  scale_fill_manual(values=c(col1,col2),labels = c("Crozier","Royds"))+
  guides(fill=guide_legend(title="Colony"),color=guide_legend(title="Colony"))+
...
# vp1 <-plotViewport(c(3,1,3,1)) ## create new vp with margins
# vp2 <- plotViewport(c(3,1,3,1))
# print(p_subcol_attr,vp = vp1)
# print(p_skua,vp=vp2)
...
concurvity(c_base_anom, full=FALSE)$estimate
...
rk=4
r_base_anom <- gam(prod_anom~s(area, k=rk) + s(pa_ratio, k=rk)+s(mean_slope,k=rk) +s(mean_aspect,k=4, bs="cc") +s(mean_windshelt,k=rk)+s(flow_acc_log1p, k=rk)+skua50, data=filter(ann_prod_anom,col=="royds"), select=TRUE, method="ML")
summary(r_base_anom)
plot.gam(r_base_anom, pages=1)
plot.gam(r_base_anom, pages=1, shade=T, residuals=T)

rb_test <- gam(prod_anom~s(area, k=rk) + s(pa_ratio, k=rk)+s(adjust_mean_elev, k=rk)+s(mean_slope,k=rk) +s(mean_aspect,k=rk, bs="cc") +s(mean_windshelt,k=rk)+skua50, data=filter(ann_prod_anom,col=="royds"), select=TRUE, method="ML")
summary(rb_test)
plot.gam(rb_test, pages=1)


summary(r_base_anom)
plot.gam(r_base_anom, pages=1, shade=T, residuals=T)
gam.check(r_base_anom) # looks pretty good, some skew in the residuals
concurvity(r_base_anom, full=FALSE)$estimate
...
# Area and aspect already dropped
# Drop pa ratio
...
# Croz Proportion explained

# c_elev_devex <- (deviance(cb2)-deviance(cb))/deviance(cb0)
# c_slope_devex <- (deviance(cb3)-deviance(cb))/deviance(cb0)
# c_wind_devex <- (deviance(cb4)-deviance(cb))/deviance(cb0)
# c_flood_devex<- (deviance(cb5)-deviance(cb))/deviance(cb0)
# c_skua_devex<- (deviance(cb6)-deviance(cb))/deviance(cb0)
...
t <- gam.vcomp(rb)
# Area, slope and wind shelter already dropped
# Area and elevation dropped already
...
# remove area
rb7 <- gam(prod_anom~ s(pa_ratio, k=rk)+s(adjust_mean_elev, k=rk)+ s(mean_slope, k=rk) +s(mean_aspect,k=rk, bs="cc") +s(mean_windshelt,k=rk)+s(flow_acc_log1p, k=rk)+skua50, data=filter(ann_prod_anom,col=="royds"),select=TRUE,method="ML", sp=rb$sp[-c(1,2)])
summary(rb7)

# remove elevation
rb8 <- gam(prod_anom~ s(area, k=rk)+s(pa_ratio, k=rk)+ s(mean_slope, k=rk) +s(mean_aspect,k=rk, bs="cc") +s(mean_windshelt,k=rk)+s(flow_acc_log1p, k=rk)+skua50, data=filter(ann_prod_anom,col=="royds"),select=TRUE,method="ML", sp=rb$sp[-c(5,6)])
summary(rb8)
...
summary(rb)$dev.expl-summary(rb9)$dev.expl
...
r_elev_devex <- 1-(deviance(rb0)-deviance(rb8))/(deviance(rb0)-deviance(rb))

# r_pa_devex <- (deviance(rb1)-deviance(rb))/deviance(rb0)
# r_slope_devex <- (deviance(rb2)-deviance(rb))/deviance(rb0)
# r_aspect_devex <- (deviance(rb3)-deviance(rb))/deviance(rb0)
# r_windshelt_devex <- (deviance(rb4)-deviance(rb))/deviance(rb0)
# r_flood_devex<- (deviance(rb5)-deviance(rb))/deviance(rb0)
# r_skua_devex<- (deviance(rb6)-deviance(rb))/deviance(rb0)

# Make table to proportion deviance explained
rb_term_devex<-c(r_area_devex,r_pa_devex,r_elev_devex,r_slope_devex,r_aspect_devex,r_windshelt_devex,r_flood_devex,r_skua_devex)
sum(rb_term_devex)
cb_term_devex<- c(0,c_pa_devex,c_elev_devex, c_slope_devex,0,c_wind_devex, c_flood_devex, c_skua_devex)
sum(cb_term_devex)

term_devex <- data.frame(term=c("Area","Perim/area_ratio","Elevation","Slope","Aspect","Wind_shelter", "flow_acc","skua"), Royds=rb_term_devex, Croz=cb_term_devex)
...
# 
rcorr(croz_ann_prod$prod_anom,croz_ann_prod$predict_base)$P
rcorr(royds_ann_prod$prod_anom,royds_ann_prod$predict_base)$P
# 
# 
...
# writeRaster(c_pred_anom, "Z:/Informatics/S031/analyses/aschmidt/subcol_var/GIS/croz/layers/croz_predict_prod_base_anom_v9.tif", format="GTiff", overwrite=FALSE)
# 
...
bqi_pred_prod <- read.csv("Z:/Informatics/S031/analyses/aschmidt/subcol_var/data/croz_bqi_all_pred_prod_v3.csv")%>%
...
med_bqi <- bqi_pred_prod%>%
  filter(bqi_cat=="m")
rcorr(med_bqi$BQI_A, med_bqi$hab_qual)
...
# including the last 6 years of breeders (still have to have bred a