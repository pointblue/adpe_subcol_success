...
c_dat <- c_ct_meas%>%
  ...
  mutate(season=factor(season),subcol=factor(subcol), m=ifelse(area_name=="m","y","n"))%>%
...
all_dat_scale <- all_dat%>%
  mutate(subcol=as.factor(subcol), col=as.factor(col),season=factor(season),flood_risk_trans=log1p(flood_risk),skua50_yn=factor(ifelse(skua50==1,"y","n")))%>%
  mutate_at(vars(active_ct,prod,area,perim,pa_ratio,mean_slope,mean_aspect,adjust_mean_elev,mean_elev,mean_wind,mean_solar,flood_risk_trans),funs(scale))
            

# Data vis ####

pairs(c_dat[,c("prod","area","pa_ratio","mean_slope","mean_aspect","adjust_mean_elev", "mean_wind", "mean_solar","flood_risk","skua50")],lower.panel = panel.smooth, upper.panel = panel.cor)


# melt data for data vis
subcol_attr <- all_dat_scale%>%
  select(-flood_risk,-active_ct,-prod,-growth,-skua50,-skua100, -mean_elev, -perim, -skua50_yn)%>%
  # dplyr::slice(1)%>%
...
k=6
gam_c_globalx <- gam(prod~s(area,by=season,k=k)+ s(pa_ratio, by=season,k=k) +s(mean_slope, by=season,k=k)+s(adjust_mean_elev,by=season,k=k)+s(mean_aspect, by=season,k=k, bs="cc") + +s(mean_wind, by=season,k=k)+s(flood_risk_trans, by=season,k=k)+ skua50_yn*season + s(subcol, bs="re"), data=c_dat_mod, select=TRUE, method="ML")

summary(gam_c_globalx)
anova(gam_c_globalx)
par(mfrow=c())
plot.gam(gam_c_globalx, pages=2, shade=TRUE, scale=0)


# Fit model with no season interactions to compare
gam_c_global <- gam(prod~s(area) + s(pa_ratio)+s(mean_slope)+ s(adjust_mean_elev) +s(mean_aspect, bs="cc") +s(mean_wind)+s(flood_risk_trans)+skua50_yn+season+s(subcol, bs="re"), data=c_dat_mod, select=TRUE, method="ML")
...
# check how compare
AICctab(gam_c_global,gam_c_globalx,nobs=247, base=TRUE, weights=TRUE, logLik=TRUE)



# interactions of interest
# other models with interactions of interest
gam_c_int1 <- gam(prod~s(area) + s(pa_ratio)+ s(pa_ratio, by=skua50_yn)+ ti(area, pa_ratio)+s(mean_slope)+ s(adjust_mean_elev) +s(mean_aspect, bs="cc") +s(mean_wind)+s(flood_risk_trans) + ti(flood_risk_trans, mean_slope)+skua50_yn+season+s(subcol, bs="re"), data=c_dat_mod, select=TRUE, method="ML")
summary(gam_c_int1)
plot.gam(gam_c_int1, pages=1,shade=TRUE)



AICctab(gam_c_global,gam_c_globalx, gam_c_int1,nobs=247, base=TRUE, weights=TRUE, logLik=TRUE)
...
# remove variables with edf = 0 and refit: slope, solar, flood
gam_c_simp <- gam(prod~s(pa_ratio)+s(adjust_mean_elev)+s(mean_wind)+skua50_yn+season+s(subcol, bs="re"), data=all_dat_scale[all_dat_scale$col=="croz",],method="ML")
summary(gam_c_simp)
...
# check how compare
AICctab(gam_c_simp,gam_c_global,gam_c_globalx, gam_c_globalx2,nobs=276, base=TRUE, weights=TRUE, logLik=TRUE)

# Try to simplify globalx model
# Remove season interaction one by one

# do not apply penalty to selecting interactions???
# remove interaction with pa_ratio
c_xs1 <- gam(prod~s(pa_ratio,k=k) + s(mean_slope,by=season,k=k)+ s(adjust_mean_elev,by=season,k=k)+s(mean_aspect, by=season,k=k,bs="cc") +s(mean_solar, by=season,k=k) +s(mean_wind, by=season,k=k)+s(flood_risk_trans, by=season,k=k)+ skua50_yn*season + s(subcol, bs="re"), data=all_dat_scale[all_dat_scale$col=="croz",], method="ML", select=TRUE)

# slope
c_xs2 <- gam(prod~s(pa_ratio, by=season,k=k) +s(mean_slope,k=k)+s(adjust_mean_elev,by=season,k=k)+s(mean_aspect, by=season,k=k,bs="cc")+ s(mean_solar, by=season,k=k) +s(mean_wind, by=season,k=k)+s(flood_risk_trans, by=season,k=k)+ skua50_yn*season + s(subcol, bs="re"), data=all_dat_scale[all_dat_scale$col=="croz",], select=TRUE, method="ML")

#elev
c_xs3 <- gam(prod~s(pa_ratio, by=season,k=k) +s(mean_slope,by=season,k=k)+s(adjust_mean_elev,k=k)+s(mean_aspect, by=season,k=k,bs="cc") +s(mean_solar, by=season,k=k) +s(mean_wind, by=season,k=k)+s(flood_risk_trans, by=season,k=k)+ skua50_yn*season + s(subcol, bs="re"), data=all_dat_scale[all_dat_scale$col=="croz",], select=TRUE, method="ML")
summary(c_xs3)
# dropping solar


#aspect
c_xs4 <- gam(prod~s(pa_ratio, by=season,k=k) +s(mean_slope,by=season,k=k)+s(adjust_mean_elev,by=season,k=k)+s(mean_aspect,k=k,bs="cc")+s(mean_solar, by=season,k=k) +s(mean_wind, by=season,k=k)+s(flood_risk_trans, by=season,k=k)+ skua50_yn*season + s(subcol, bs="re"), data=all_dat_scale[all_dat_scale$col=="croz",], select=TRUE, method="ML")

# solar
c_xs5 <- gam(prod~s(pa_ratio,by=season,k=k) +s(mean_slope,by=season,k=k)+s(adjust_mean_elev,by=season,k=k)+s(mean_aspect,by=season,k=k, bs="cc") +s(mean_solar,k=k) +s(mean_wind,by=season,k=k)+s(flood_risk_trans, by=season,k=k)+ skua50_yn*season + s(subcol, bs="re"), data=all_dat_scale[all_dat_scale$col=="croz",], select=TRUE, method="ML")

# wind
c_xs6 <- gam(prod~s(pa_ratio, by=season,k=k) +s(mean_slope,by=season,k=k)+s(adjust_mean_elev,by=season,k=k)+s(mean_aspect,by=season,k=k,bs="cc") +s(mean_solar, by=season,k=k) +s(mean_wind,k=k) +s(flood_risk_trans,by=season, k=k)+ skua50_yn*season + s(subcol, bs="re"), data=all_dat_scale[all_dat_scale$col=="croz",], select=TRUE, method="ML")

# skua
c_xs7 <- gam(prod~s(pa_ratio, by=season,k=k) +s(mean_slope,by=season,k=k)+s(adjust_mean_elev,by=season,k=k)+s(mean_aspect,by=season,k=k,bs="cc") +s(mean_solar, by=season,k=k) +s(mean_wind,by=season,k=k) +s(flood_risk_trans,by=season,k=k)+ skua50_yn+season + s(subcol, bs="re"), data=all_dat_scale[all_dat_scale$col=="croz",], select=TRUE, method="ML")

# check how compare
AICctab(gam_c_simp,gam_c_global,gam_c_globalx, gam_c_globalx2, c_xs1, c_xs2, c_xs3, c_xs4, c_xs5, c_xs6,c_xs7, nobs=276, base=TRUE, weights=TRUE, logLik=TRUE)

# removing season interaction with slope and wind both improved AIC slightly

# round 2
# start with model with interaction with elevation removed and remove additional interactions
# remove interaction with pa_ratio
c_xs3.1 <- gam(prod~s(pa_ratio,k=k) +s(mean_slope,by=season,k=k)+s(adjust_mean_elev,k=k)+s(mean_aspect, by=season,k=k,bs="cc") +s(mean_solar, by=season, k=k) +s(mean_wind, by=season,k=k)+s(flood_risk_trans, by=season,k=k)+ skua50_yn*season + s(subcol, bs="re"), data=all_dat_scale[all_dat_scale$col=="croz",], select=TRUE, method="ML")

#slope
c_xs3.2 <- gam(prod~s(pa_ratio, by=season,k=k) + s(mean_slope,k=k)+s(adjust_mean_elev,k=k)+s(mean_aspect, by=season,k=k,bs="cc")+s(mean_solar, by=season, k=k)+s(mean_wind, by=season,k=k)+s(flood_risk_trans, by=season,k=k)+ skua50_yn*season + s(subcol, bs="re"), data=all_dat_scale[all_dat_scale$col=="croz",], select=TRUE, method="ML")
summary(c_xs3.2)

# aspect
c_xs3.3 <- gam(prod~s(pa_ratio, by=season,k=k) +s(mean_slope, by=season,k=k)+s(adjust_mean_elev,k=k)+s(mean_aspect,k=k,bs="cc")+s(mean_solar, by=season,k=k) + s(mean_wind, by=season,k=k)+s(flood_risk_trans, by=season,k=k)+ skua50_yn*season + s(subcol, bs="re"), data=all_dat_scale[all_dat_scale$col=="croz",], select=TRUE, method="ML")
summary(c_xs1.3)
plot.gam(c_xs1.3, pages=1, shade=T)

# solar
c_xs3.4<- gam(prod~s(pa_ratio, by=season,k=k) +s(mean_slope, by=season,k=k)+s(adjust_mean_elev,k=k)+s(mean_aspect,by=season,k=k,bs="cc")+s(mean_solar,k=k) + s(mean_wind, by=season,k=k)+s(flood_risk_trans, by=season,k=k)+ skua50_yn*season + s(subcol, bs="re"), data=all_dat_scale[all_dat_scale$col=="croz",], select=TRUE, method="ML")

#wind
c_xs3.5<- gam(prod~s(pa_ratio, by=season,k=k) +s(mean_slope, by=season,k=k)+s(adjust_mean_elev,k=k)+s(mean_aspect,by=season,k=k,bs="cc")+s(mean_solar, by=season, k=k) + s(mean_wind,k=k)+s(flood_risk_trans, by=season,k=k)+ skua50_yn*season + s(subcol, bs="re"), data=all_dat_scale[all_dat_scale$col=="croz",], select=TRUE, method="ML")

#flood
c_xs3.6 <- gam(prod~s(pa_ratio, by=season,k=k) +s(mean_slope,by=season,k=k)+s(adjust_mean_elev,k=k)+s(mean_aspect, by=season,k=k,bs="cc") + s(mean_solar, by=season, k=k)+s(mean_wind, by=season,k=k)+s(flood_risk_trans,k=k)+ skua50_yn*season + s(subcol, bs="re"), data=all_dat_scale[all_dat_scale$col=="croz",], select=TRUE, method="ML")

#skua
c_xs3.7 <- gam(prod~s(pa_ratio, by=season,k=k) +s(mean_slope, by=season,k=k) +s(adjust_mean_elev,k=k)+s(mean_aspect, by=season,k=k,bs="cc") + s(mean_solar, by=season, k=k) +s(mean_wind, by=season,k=k)+s(flood_risk_trans, by=season,k=k)+ skua50_yn+season + s(subcol, bs="re"), data=all_dat_scale[all_dat_scale$col=="croz",], select=TRUE, method="ML")
summary(c_xs3.7)


AICctab(gam_c_simp,gam_c_global,gam_c_globalx, gam_c_globalx2,gam_c_xseas,c_xs1, c_xs2, c_xs3, c_xs4, c_xs5, c_xs6,c_xs3.1,c_xs3.2,c_xs3.3,c_xs3.4,c_xs3.5,c_xs3.6,c_xs3.7, nobs=276, base=TRUE, weights=TRUE, logLik=TRUE)

# model with interaction with slope and wind removed is best

# round 3
# continue removing additional season interactions
# remove pa_ratio
c_xs3.7.1 <- gam(prod~s(pa_ratio,k=k)+s(adjust_mean_elev,k=k) +s(mean_slope, by=season,k=k) +s(mean_aspect, by=season,k=k, bs="cc") + s(mean_solar, by=season, k=k) +s(mean_wind, by=season,k=k)+s(flood_risk_trans, by=season,k=k)+ skua50_yn+season + s(subcol, bs="re"), data=all_dat_scale[all_dat_scale$col=="croz",], select=TRUE, method="ML")
summary(c_xs3.7.1)
plot.gam(c_xs3.7.1, pages=2, shade=TRUE)

#slope
c_xs3.7.2 <- gam(prod~s(pa_ratio, by=season,k=k) +s(mean_slope,k=k) +s(adjust_mean_elev,k=k)+s(mean_aspect, by=season,k=k, bs="cc") + s(mean_solar, by=season, k=k) +s(mean_wind, by=season,k=k)+s(flood_risk_trans, by=season,k=k)+ skua50_yn+season + s(subcol, bs="re"), data=all_dat_scale[all_dat_scale$col=="croz",], select=TRUE, method="ML")

#aspect
c_xs3.7.3 <- gam(prod~s(pa_ratio, by=season,k=k) +s(mean_slope, by=season,k=k) +s(adjust_mean_elev,k=k)+s(mean_aspect,k=k, bs="cc") + s(mean_solar, by=season, k=k) +s(mean_wind, by=season,k=k)+s(flood_risk_trans, by=season,k=k)+ skua50_yn+season + s(subcol, bs="re"), data=all_dat_scale[all_dat_scale$col=="croz",], select=TRUE, method="ML")
summary(c_xs3.7.3)

# solar
c_xs3.7.4 <- gam(prod~s(pa_ratio, by=season,k=k) +s(mean_slope, by=season,k=k) +s(adjust_mean_elev,k=k)+s(mean_aspect, by=season,k=k, bs="cc") + s(mean_solar, k=k) +s(mean_wind, by=season,k=k)+s(flood_risk_trans, by=season,k=k)+ skua50_yn+season + s(subcol, bs="re"), data=all_dat_scale[all_dat_scale$col=="croz",], select=TRUE, method="ML")

# wind
c_xs3.7.5 <- gam(prod~s(pa_ratio, by=season,k=k) +s(mean_slope, by=season,k=k) +s(adjust_mean_elev,k=k)+s(mean_aspect, by=season,k=k, bs="cc") + s(mean_solar, by=season, k=k) +s(mean_wind,k=k)+s(flood_risk_trans, by=season,k=k)+ skua50_yn+season + s(subcol, bs="re"), data=all_dat_scale[all_dat_scale$col=="croz",], select=TRUE, method="ML")

# flood
c_xs3.7.6 <- gam(prod~s(pa_ratio, by=season,k=k) +s(mean_slope, by=season,k=k) +s(adjust_mean_elev,k=k)+s(mean_aspect, by=season,k=k, bs="cc") + s(mean_solar, by=season, k=k) +s(mean_wind,by=season,k=k)+s(flood_risk_trans,k=k)+ skua50_yn+season + s(subcol, bs="re"), data=all_dat_scale[all_dat_scale$col=="croz",], select=TRUE, method="ML")


AICctab(gam_c_simp,gam_c_global,gam_c_globalx, gam_c_globalx2,gam_c_xseas,c_xs1, c_xs2, c_xs3, c_xs4, c_xs5, c_xs6,c_xs3.1,c_xs3.2,c_xs3.3,c_xs3.4,c_xs3.5,c_xs3.6,c_xs3.7,c_xs3.7.1,c_xs3.7.2,c_xs3.7.3,c_xs3.7.4,c_xs3.7.5,c_xs3.7.6, nobs=276, base=TRUE, weights=TRUE, logLik=TRUE)

# round 4
# slope
c_xs3.7.3.1 <- gam(prod~s(pa_ratio,by=season,k=k, bs="fs") +s(mean_slope,k=k) +s(adjust_mean_elev,k=k)+s(mean_aspect,k=k,  bs="cc") + s(mean_solar, by=season,k=k, bs="fs") +s(mean_wind, by=season,k=k, bs="fs")+s(flood_risk_trans, by=season,k=k, bs="fs")+ skua50_yn+season + s(subcol, bs="re"), data=all_dat_scale[all_dat_scale$col=="croz",], select=TRUE, method="ML")
summary(c_xs3.7.3.1)
# aspect
c_xs3.7.3.2 <- gam(prod~s(pa_ratio,by=season,k=k, bs="fs") +s(mean_slope, by=season,k=k, bs="fs") +s(adjust_mean_elev,k=k)+s(mean_aspect,k=k,  bs="cc") + s(mean_solar, by=season,k=k, bs="fs") +s(mean_wind, by=season,k=k, bs="fs")+s(flood_risk_trans, by=season,k=k, bs="fs")+ skua50_yn+season + s(subcol, bs="re"), data=all_dat_scale[all_dat_scale$col=="croz",], select=TRUE, method="ML")
# solar
c_xs3.7.3.3 <- gam(prod~s(pa_ratio,by=season,k=k, bs="fs") +s(mean_slope, by=season,k=k, bs="fs") +s(adjust_mean_elev,k=k)+s(mean_aspect,k=k, bs="cc") + s(mean_solar, k=k) +s(mean_wind,by=season,k=k, bs="fs")+s(flood_risk_trans,by=season,k=k, bs="fs")+ skua50_yn+season + s(subcol, bs="re"), data=all_dat_scale[all_dat_scale$col=="croz",], select=TRUE, method="ML")
# wind
c_xs3.7.3.4 <- gam(prod~s(pa_ratio,by=season,k=k, bs="fs") +s(mean_slope, by=season,k=k, bs="fs") +s(adjust_mean_elev,k=k)+s(mean_aspect,k=k, bs="cc") + s(mean_solar, by=season,k=k, bs="fs") +s(mean_wind,k=k)+s(flood_risk_trans, by=season,k=k, bs="fs")+ skua50_yn+season + s(subcol, bs="re"), data=all_dat_scale[all_dat_scale$col=="croz",], select=TRUE, method="ML")
# flood
c_xs3.7.3.5 <- gam(prod~s(pa_ratio,by=season,k=k, bs="fs") +s(mean_slope,by=season,k=k, bs="fs") +s(adjust_mean_elev,k=k)+s(mean_aspect,k=k, bs="cc") + s(mean_solar, by=season,k=k, bs="fs") +s(mean_wind, by=season,k=k, bs="fs")+s(flood_risk_trans,k=k)+ skua50_yn+season + s(subcol, bs="re"), data=all_dat_scale[all_dat_scale$col=="croz",], select=TRUE, method="ML")


AICctab(gam_c_simp,gam_c_global,gam_c_globalx, gam_c_globalx2,gam_c_xseas,c_xs1, c_xs2, c_xs3, c_xs4, c_xs5, c_xs6,c_xs3.1,c_xs3.2,c_xs3.3,c_xs3.4,c_xs3.5,c_xs3.6,c_xs3.7,c_xs3.7.1,c_xs3.7.2,c_xs3.7.3,c_xs3.7.4,c_xs3.7.5,c_xs3.7.6,c_xs3.7.3.1,c_xs3.7.3.2,c_xs3.7.3.3,c_xs3.7.3.4,c_xs3.7.3.5, nobs=276, base=TRUE, weights=TRUE, logLik=TRUE)

# round 5
# pa_ratio
c_xs3.7.3.5.1 <- gam(prod~s(pa_ratio,k=k) +s(mean_slope,by=season,k=k, bs="fs") +s(adjust_mean_elev,k=k)+s(mean_aspect,k=k, bs="cc") + s(mean_solar, by=season,k=k, bs="fs") +s(mean_wind, by=season,k=k, bs="fs")+s(flood_risk_trans,k=k)+ skua50_yn+season + s(subcol, bs="re"), data=all_dat_scale[all_dat_scale$col=="croz",], select=TRUE, method="ML")
#slope
c_xs3.7.3.5.2 <- gam(prod~s(pa_ratio,by=season,k=k, bs="fs") +s(mean_slope,k=k) +s(adjust_mean_elev,k=k)+s(mean_aspect,k=k, bs="cc") + s(mean_solar, by=season,k=k, bs="fs") +s(mean_wind, by=season,k=k, bs="fs")+s(flood_risk_trans,k=k)+ skua50_yn+season + s(subcol, bs="re"), data=all_dat_scale[all_dat_scale$col=="croz",], select=TRUE, method="ML")

#solar
c_xs3.7.3.5.3 <- gam(prod~s(pa_ratio,by=season,k=k, bs="fs") +s(mean_slope,by=season,k=k, bs="fs") +s(adjust_mean_elev,k=k)+s(mean_aspect,k=k, bs="cc") + s(mean_solar,k=k) +s(mean_wind, by=season,k=k, bs="fs")+s(flood_risk_trans,k=k)+ skua50_yn+season + s(subcol, bs="re"), data=all_dat_scale[all_dat_scale$col=="croz",], select=TRUE, method="ML")

#wind
c_xs3.7.3.5.4 <- gam(prod~s(pa_ratio,by=season,k=k, bs="fs") +s(mean_slope,by=season,k=k, bs="fs") +s(adjust_mean_elev,k=k)+s(mean_aspect,k=k, bs="cc") + s(mean_solar, by=season,k=k, bs="fs") +s(mean_wind,k=k)+s(flood_risk_trans,k=k)+ skua50_yn+season + s(subcol, bs="re"), data=all_dat_scale[all_dat_scale$col=="croz",], select=TRUE, method="ML")


AICctab(gam_c_simp,gam_c_global,gam_c_globalx, gam_c_globalx2,gam_c_xseas,c_xs1, c_xs2, c_xs3, c_xs4, c_xs5, c_xs6,c_xs3.1,c_xs3.2,c_xs3.3,c_xs3.4,c_xs3.5,c_xs3.6,c_xs3.7,c_xs3.7.1,c_xs3.7.2,c_xs3.7.3,c_xs3.7.4,c_xs3.7.5,c_xs3.7.6,c_xs3.7.3.1,c_xs3.7.3.2,c_xs3.7.3.3,c_xs3.7.3.4,c_xs3.7.3.5,c_xs3.7.3.5.1,c_xs3.7.3.5.2,c_xs3.7.3.5.3,c_xs3.7.3.5.4, nobs=276, base=TRUE, weights=TRUE, logLik=TRUE)

# round 6
# pa_ratio
c_xs3.7.3.5.2.1 <- gam(prod~s(pa_ratio,k=k) +s(mean_slope,k=k) +s(adjust_mean_elev,k=k)+s(mean_aspect,k=k, bs="cc") + s(mean_solar, by=season,k=k, bs="fs") +s(mean_wind, by=season,k=k, bs="fs")+s(flood_risk_trans,k=k)+ skua50_yn+season + s(subcol, bs="re"), data=all_dat_scale[all_dat_scale$col=="croz",], select=TRUE, method="ML")
# solar
c_xs3.7.3.5.2.2 <- gam(prod~s(pa_ratio,by=season,k=k, bs="fs") +s(mean_slope,k=k) +s(adjust_mean_elev,k=k)+s(mean_aspect,k=k, bs="cc") + s(mean_solar,k=k) +s(mean_wind, by=season,k=k, bs="fs")+s(flood_risk_trans,k=k)+ skua50_yn+season + s(subcol, bs="re"), data=all_dat_scale[all_dat_scale$col=="croz",], select=TRUE, method="ML")

# wind
c_xs3.7.3.5.2.3 <- gam(prod~s(pa_ratio,by=season,k=k, bs="fs") +s(mean_slope,k=k) +s(adjust_mean_elev,k=k)+s(mean_aspect,k=k, bs="cc") + s(mean_solar, by=season,k=k, bs="fs") +s(mean_wind,k=k)+s(flood_risk_trans,k=k)+ skua50_yn+season + s(subcol, bs="re"), data=all_dat_scale[all_dat_scale$col=="croz",], select=TRUE, method="ML")

AICctab(gam_c_simp,gam_c_global,gam_c_globalx, gam_c_globalx2,gam_c_xseas,c_xs1, c_xs2, c_xs3, c_xs4, c_xs5, c_xs6,c_xs3.1,c_xs3.2,c_xs3.3,c_xs3.4,c_xs3.5,c_xs3.6,c_xs3.7,c_xs3.7.1,c_xs3.7.2,c_xs3.7.3,c_xs3.7.4,c_xs3.7.5,c_xs3.7.6,c_xs3.7.3.1,c_xs3.7.3.2,c_xs3.7.3.3,c_xs3.7.3.4,c_xs3.7.3.5,c_xs3.7.3.5.1,c_xs3.7.3.5.2,c_xs3.7.3.5.3,c_xs3.7.3.5.4,c_xs3.7.3.5.2.1, c_xs3.7.3.5.2.2, c_xs3.7.3.5.2.3, nobs=276, base=TRUE, weights=TRUE, logLik=TRUE)

# round 7
# pa_ratio
c_xs3.7.3.5.2.3.1 <- gam(prod~s(pa_ratio,k=k) +s(mean_slope,k=k) +s(adjust_mean_elev,k=k)+s(mean_aspect,k=k, bs="cc") + s(mean_solar, by=season,k=k, bs="fs") +s(mean_wind,k=k)+s(flood_risk_trans,k=k)+ skua50_yn+season + s(subcol, bs="re"), data=all_dat_scale[all_dat_scale$col=="croz",], select=TRUE, method="ML")

#solar
c_xs3.7.3.5.2.3.2 <- gam(prod~s(pa_ratio,by=season,k=k, bs="fs") +s(mean_slope,k=k) +s(adjust_mean_elev,k=k)+s(mean_aspect,k=k, bs="cc") + s(mean_solar,k=k, bs="fs") +s(mean_wind,k=k)+s(flood_risk_trans,k=k)+ skua50_yn+season + s(subcol, bs="re"), data=all_dat_scale[all_dat_scale$col=="croz",], select=TRUE, method="ML")

AICctab(gam_c_global,gam_c_globalx,c_xs1, c_xs2, c_xs3, c_xs4, c_xs5, c_xs6,c_xs3.1,c_xs3.2,c_xs3.3,c_xs3.4,c_xs3.5,c_xs3.6,c_xs3.7,c_xs3.7.1,c_xs3.7.2,c_xs3.7.3,c_xs3.7.4,c_xs3.7.5,c_xs3.7.6,c_xs3.7.3.1,c_xs3.7.3.2,c_xs3.7.3.3,c_xs3.7.3.4,c_xs3.7.3.5,c_xs3.7.3.5.1,c_xs3.7.3.5.2,c_xs3.7.3.5.3,c_xs3.7.3.5.4,c_xs3.7.3.5.2.1, c_xs3.7.3.5.2.2, c_xs3.7.3.5.2.3,c_xs3.7.3.5.2.3.1,c_xs3.7.3.5.2.3.2, nobs=276, base=TRUE, weights=TRUE, logLik=TRUE)

summary(c_xs3.7.3.5.2.3.2)
plot.gam(c_xs3.7.3.5.2.3.2, page=1,shade=T)
...
c_newdat<- cMod_dat[hold_out_index,]

m_pred <- predict.gam(gam_c_globalx,exclude="subcol",newdata=c_newdat)

rcorr(c_newdat$prod,m_pred)
plot(c_newdat$prod,m_pred)
