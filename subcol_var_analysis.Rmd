...
# Royds
r_ct_meas <- read_csv("data/royds_selected_meas_ct_all_v6.csv")
...
# wind shelt to -0.016
r_dat[r_dat$subcol=="1b","mean_windshelt"]=-0.00120
# aspect to 253.100
...
rk=4
r_base_anom <- gam(prod_anom~s(area, k=rk) + s(pa_ratio, k=rk)+s(adjust_mean_elev, k=rk)+s(mean_slope,k=rk) +s(mean_aspect,k=4, bs="cc") +s(mean_windshelt,k=rk)+s(flow_acc_log1p, k=rk)+skua50, data=filter(ann_prod_anom,col=="royds"), select=TRUE, method="ML")
summary(r_base_anom)
plot.gam(r_base_anom)

rb_test <- gam(prod_anom~s(area, k=rk) + s(pa_ratio, k=rk)+s(adjust_mean_elev, k=rk)+s(mean_slope,k=rk) +s(mean_aspect,k=rk, bs="cc") +s(mean_windshelt,k=rk)+s(flow_acc_log1p, k=3)+skua50, data=filter(ann_prod_anom,col=="royds"), select=TRUE, method="ML")
summary(rb_test)
...
rb_term_devex<-c(0,r_pa_devex,0,r_slope_devex,r_aspect_devex,r_windshelt_devex,r_flood_devex,r_skua_devex)
sum(rb_term_devex)
...
# writeRaster(r_pred_anom, "Z:/Informatics/S031/analyses/aschmidt/subcol_var/GIS/royds/layers/royds_predict_prod_base_anom_v1.tif", format="GTiff", overwrite=FALSE)
