...
c_dat$adjust_mean_aspect<-ifelse(!is.na(c_dat$mean_aspect)&c_dat$mean_aspect>300, c_dat$mean_aspect-360,c_dat$mean_aspect)
# hist(c_dat$adjust_mean_aspect)
# hist(c_dat$mean_aspect)
...
subcol_attr <- all_dat%>%
  select(-season,-active_ct,-prod,-growth,-area_name,-skua50,-skua100, -mean_elev, -perim)%>%
  group_by(col, subcol)%>%
  dplyr::slice(1)%>%
...
gam_c_global <- gam(prod~s(area, k=k,bs="cr")+s(pa_ratio, k=k, bs="cr")+s(mean_slope,k=k, bs="cr")+s(adjust_mean_elev,k=k, bs="cr")+s(adjust_mean_aspect,k=10, bs="cc")+s(mean_solar,k=k, bs="cr")+s(mean_wind,k=k, bs="cr")+s(flood_risk, k=k, bs="cr")+skua50+season+s(subcol,k=k, bs="re"),data=all_dat_scale[all_dat_scale$col=="croz",],select=TRUE, method="ML")
...
# lm_c_global <- lm(prod~area+pa_ratio+I(pa_ratio^2)+mean_slope+adjust_mean_elev+I(adjust_mean_elev^2)+mean_wind+skua50+season,data=all_dat_scale[all_dat_scale$col=="croz",])
# summary(lm_c_global)
# AICctab(gam_c_global,lm_c_global,nobs=204, base=TRUE, weights=TRUE, logLik=TRUE)

# k=7
# gam_c_growth_global <- gam(growth~s(area, k=k, bs="cs")+s(pa_ratio, k=k, bs="cs")+s(mean_slope,k=k, bs="cs")+s(adjust_mean_elev,k=k, bs="cs")+s(adjust_mean_aspect,k=10, bs="cs")+s(mean_solar,k=k, bs="cs")+s(mean_wind,k=k, bs="cs")+s(flood_risk, k=k, bs="cs")+skua50+season+s(subcol,k=k, bs="re"),data=c_dat_scale, method="ML")
# summary(gam_c_growth_global)
# plot.gam(gam_c_growth_global, pages=1, scale=0,shade=TRUE)

# remove variables with edf = 0 and refit: solar, flood
gam_c_simp <- gam(prod~s(area,k=k,bs="cr")+s(pa_ratio, k=k, bs="cr")+s(mean_slope,k=k, bs="cr")+s(adjust_mean_elev,k=k, bs="cr")+s(adjust_mean_aspect,k=10, bs="cc")+s(mean_wind,k=k, bs="cr")+skua50+season+s(subcol,k=k, bs="re"), data=all_dat_scale[all_dat_scale$col=="croz",], select=TRUE,method="ML")
summary(gam_c_simp)
plot.gam(gam_c_simp, shade=TRUE, pages=1, scale=0)


# # check how compare
# AICctab(gam_c_global,gam_c_simp,nobs=204, base=TRUE, weights=TRUE, logLik=TRUE)

# model simp with season interaction in some variables
# did not include interactions with area, and pa_ratio because don't expect those to vary
gam_c_xseas <- gam(prod~s(area,by=season,k=k, bs="cr")+s(pa_ratio,by=season,k=k, bs="cr")+s(mean_slope, by=season,k=k, bs="cr")+s(adjust_mean_elev,by=season,k=k, bs="cr")+s(adjust_mean_aspect,by=season,k=10, bs="cc")+s(mean_wind,by=season,k=k, bs="cr")+skua50*season+s(subcol,k=k, bs="re"),data=all_dat_scale[all_dat_scale$col=="croz",],select=TRUE, method="ML")
summary(gam_c_xseas)
anova(gam_c_xseas)
plot.gam(gam_c_xseas,pages=2,shade=TRUE, scale=0)

AICctab(gam_c_simp,gam_c_global,gam_c_xseas,nobs=204, base=TRUE, weights=TRUE, logLik=TRUE)
...
k=5
gam_r_global <- gam(prod~s(area, k=k, bs="cr")+s(pa_ratio,k=k, bs="cr")+s(mean_slope,k=k, bs="cr")+s(adjust_mean_elev,k=k, bs="cr")+s(mean_aspect,k=10, bs="cc")+s(mean_solar,k=k, bs="cr")+s(mean_wind,k=k, bs="cr")+s(flood_risk,k=3, bs="cr")+s(subcol,k=k,bs="re")+skua50+season, data=all_dat_scale[all_dat_scale$col=="royds",],select=TRUE, method="ML")
...
k=8
gam_r_simp1 <- gam(prod~s(pa_ratio,k=k, bs="cs")+s(mean_slope,k=k, bs="cs")+s(adjust_mean_elev,k=k, bs="cs")+s(mean_solar,k=k, bs="cs")+skua50+season, data=all_dat_scale[all_dat_scale$col=="royds",], method="ML")
summary(gam_r_simp1)
...
# skua interaction no supported



```
Predict Breeding success to whole colony
```{r Predict BS}
library(raster)
library(Hmisc)
# fit model with unscaled data and "REML" for prediction
gam_c_simp1_pred <- gam(prod~s(area, k=k, bs="cr")+s(pa_ratio, k=k, bs="cr")+s(mean_slope,k=k, bs="cr")+s(adjust_mean_elev,k=k, bs="cr")+s(mean_wind,k=k, bs="cr")+skua50+season+s(subcol,k=k, bs="re"), data=all_dat[all_dat$col=="croz",], method="REML")
...
# select only one year # remove s3 because it's an outlier
  filter(season%in%c("1415","1516","1617","1718"),!subcol=="s3",col=="croz",subcol%in%factor(m_index))%>%
  mutate(subcol=factor("b33"))

m_pred <- predict.gam(gam_c_simp1_pred,newdata=c_newdat)
c_newdat$m_pred <- m_pred
rcorr(c_newdat$prod,c_newdat$m_pred)
plot(c_newdat$prod,c_newdat$m_pred)
names(m_pred)
